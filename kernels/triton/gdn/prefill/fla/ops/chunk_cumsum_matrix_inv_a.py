...
def chunk_cumsum_matrix_inv_a_64_kernel(
    k,
    beta,
    g,
    g_cumsum,
...
def chunk_cumsum_matrix_inv_a_fwd(
    k: torch.Tensor,
    beta: torch.Tensor,
    g: torch.Tensor,
    ...
    # Fused local cumsum(g) + chunk_matrix_inv_a for BT=64.
    B, T, Hg, K = k.shape
    H = beta.shape[-1]
    BT = chunk_size
    assert BT == 64, (
        "chunk_cumsum_matrix_inv_a_fwd currently only supports chunk_size=64"
    )
    ...
    chunk_cumsum_matrix_inv_a_64_kernel[(NT, B * H)](
        k=k,
        beta=beta,
        g=g,
        g_cumsum=g_cumsum,
