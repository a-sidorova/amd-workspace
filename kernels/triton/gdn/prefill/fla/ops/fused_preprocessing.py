...
def fused_preprocessing_64_kernel(
    k,
    v,
    beta,
    g,
...
def fused_preprocessing_fwd(
    ...
    """
    Fused preprocessing path:
      local_cumsum(g) -> chunk_matrix_inv_a -> recompute(w, u).
    ...
    V = v.shape[-1]
    BT = chunk_size
    assert BT == 64, "fused_preprocessing_fwd currently only supports chunk_size=64"
    assert k.dtype == v.dtype, "fused_preprocessing_fwd expects k and v to share dtype"
    ...
    fused_preprocessing_64_kernel[(NT, B * H)](
        k=k,
        v=v,
        beta=beta,
        g=g,
