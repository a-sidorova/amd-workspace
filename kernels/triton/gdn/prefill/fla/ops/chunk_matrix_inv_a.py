...
def chunk_matrix_inv_a_64_kernel(
    k,
    beta,
    g,
    A,
    ...
    b_beta_4 = tl.load(p_beta_4, boundary_check=(0,))
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        b_k_1 = tl.load(p_k_1, boundary_check=(0, 1))
        b_k_2 = tl.load(p_k_2, boundary_check=(0, 1))
        b_k_3 = tl.load(p_k_3, boundary_check=(0, 1))
        b_k_4 = tl.load(p_k_4, boundary_check=(0, 1))
    ...
    if USE_G:
        p_g_1 = tl.make_block_ptr(g + bos * H + i_h, (T,), (H,), (i_t * BT + 0,), (16,), (0,))
        p_g_2 = tl.make_block_ptr(g + bos * H + i_h, (T,), (H,), (i_t * BT + 16,), (16,), (0,))
        p_g_3 = tl.make_block_ptr(g + bos * H + i_h, (T,), (H,), (i_t * BT + 32,), (16,), (0,))
        p_g_4 = tl.make_block_ptr(g + bos * H + i_h, (T,), (H,), (i_t * BT + 48,), (16,), (0,))
        b_g_1 = tl.load(p_g_1, boundary_check=(0,))
        b_g_2 = tl.load(p_g_2, boundary_check=(0,))
        b_g_3 = tl.load(p_g_3, boundary_check=(0,))
        b_g_4 = tl.load(p_g_4, boundary_check=(0,))
    ...
    # Invert (I + A) in registers for fixed 64x64 chunks using 16x16 block solves.
    o_i = tl.arange(0, 16)
...
def chunk_matrix_inv_a_fwd(
    ...
    # This fused kernel computes A = beta * K * K^T and inverts (I + A) for BT=64.
    B, T, Hg, K = k.shape
    H = beta.shape[-1]
    BT = chunk_size
    assert BT == 64, "chunk_matrix_inv_a_fwd currently only supports chunk_size=64"
    chunk_indices = (
    ...
    chunk_matrix_inv_a_64_kernel[(NT, B * H)](
        k=k,
        g=g,
        beta=beta,
        A=A,
...
# Backward compatible alias during migration to fused naming.
chunk_scaled_dot_kkt_fwd = chunk_matrix_inv_a_fwd
